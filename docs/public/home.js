(function() {
  ...
  function scrambleText(el, text, opts) {
    ...
    if (dir === 'right') order.reverse();
      ...
      var mid = len / 2;
      order.sort(function(a, b) { return Math.abs(a - mid) - Math.abs(b - mid); });
    } else if (dir === 'random') {
    ...
    function frame(now) {
      ...
      for (var ri = Object.keys(revealed).length; ri < count && ri < order.length; ri++) revealed[order[ri]] = true;
      ...
      for (var ci = 0; ci < len; ci++) {
        if (revealed[ci]) out += text[ci];
          ...
          if (cursor && oi >= cursorStart && oi < cursorEnd) out += cursor[(oi - cursorStart) % cursor.length];
  ...
  function initPlayground() {
    ...
    document.querySelectorAll('[data-chars]').forEach(function(btn) {
      btn.addEventListener('click', function() {
        pgCharset = btn.getAttribute('data-chars');
        document.querySelectorAll('[data-chars]').forEach(function(c) { c.classList.toggle('is-active', c === btn); });
        runPlayground();
      });
    });
    ...
    document.querySelectorAll('[data-dir]').forEach(function(btn) {
      btn.addEventListener('click', function() {
        ...
        document.querySelectorAll('[data-dir]').forEach(function(c) { c.classList.toggle('is-active', c === btn); });
        runPlayground();
      });
    });

    pgDuration.addEventListener('input', function() {
      pgDurationVal.textContent = pgDuration.value + 'ms';
    });

    pgText.addEventListener('input', function() {
      pgOutput.textContent = pgText.value || 'Hello, Scrambl!';
    });

    pgText.addEventListener('keydown', function(e) {
    ...
    function runPlayground() {
      ...
      var dur = parseInt(pgDuration.value, 10) || 800;
      var cursor = pgCursor.value || '';
      scrambleText(pgOutput, text, { chars: chars, cursor: cursor, duration: dur, from: pgDir, fixedCells: true, cellClass: 'pg-glyph' });
    }
  }
  ...
  function init() {
    ...
    scrambleText(headTop, 'Scramble', { chars: CHARS.blocks, cursor: CURSORS.blocks, duration: 900, from: 'left', fixedCells: true });
    scrambleText(headBottom, 'text', { chars: CHARS.blocks, cursor: CURSORS.blocks, duration: 1000, from: 'right', fixedCells: true });

    var brandEl = document.querySelector('.scramble-el');
    if (brandEl) scrambleText(brandEl, 'Scrambl', { chars: CHARS.blocks, cursor: CURSORS.blocks, duration: 700 });

    headTop.addEventListener('pointerenter', function() {
      scrambleText(headTop, 'Scramble', { chars: CHARS[activeMode], cursor: CURSORS[activeMode], duration: 500, fixedCells: true });
    });
    headBottom.addEventListener('pointerenter', function() {
      scrambleText(headBottom, 'text', { chars: CHARS[activeMode], cursor: CURSORS[activeMode], duration: 500, fixedCells: true });
    });
    ...
    chipBtns.forEach(function(btn) {
      btn.addEventListener('click', function() {
        ...
        chipBtns.forEach(function(c) { c.classList.toggle('is-active', c === btn); });
        scrambleText(headTop, 'Scramble', { chars: CHARS[activeMode], cursor: CURSORS[activeMode], duration: 700, from: 'left', fixedCells: true });
        scrambleText(headBottom, 'text', { chars: CHARS[activeMode], cursor: CURSORS[activeMode], duration: 780, from: 'right', fixedCells: true });
      });
    });
    ...
    document.querySelectorAll('[data-fw]').forEach(function(btn) {
      btn.addEventListener('click', function() {
        var fw = btn.getAttribute('data-fw');
        document.querySelectorAll('[data-fw]').forEach(function(b) { b.classList.toggle('is-active', b === btn); });
